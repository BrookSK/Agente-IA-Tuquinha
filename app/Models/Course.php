...
class Course
{
    ...
    {
        $pdo = Database::getConnection();
        $stmt = $pdo->prepare('INSERT INTO courses (owner_user_id, title, slug, short_description, description, image_path, is_paid, price_cents, allow_plan_access_only, allow_public_purchase, is_active)
            VALUES (:owner_user_id, :title, :slug, :short_description, :description, :image_path, :is_paid, :price_cents, :allow_plan_access_only, :allow_public_purchase, :is_active)');
        $stmt->execute([
