...
class Personality
{
    ...
    {
        $pdo = Database::getConnection();
        $stmt = $pdo->prepare('INSERT INTO personalities (name, area, slug, prompt, image_path, is_default, active) VALUES (:name, :area, :slug, :prompt, :image_path, :is_default, :active)');
        $stmt->execute([
            'name' => $data['name'],
            'area' => $data['area'],
            'slug' => $data['slug'],
    ...
    {
        $pdo = Database::getConnection();
        $stmt = $pdo->prepare('UPDATE personalities SET name = :name, area = :area, slug = :slug, prompt = :prompt, image_path = :image_path, is_default = :is_default, active = :active WHERE id = :id');
        $stmt->execute([
            'id' => $id,
            'name' => $data['name'],
            'area' => $data['area'],
