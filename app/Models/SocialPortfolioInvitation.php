...
{
    public static function create(int $ownerUserId, int $inviterUserId, string $invitedEmail, ?string $invitedName, string $role, string $token): int
    {
        $role = in_array($role, ['read', 'edit'], true) ? $role : 'read';

        $pdo = Database::getConnection();
        $stmt = $pdo->prepare('INSERT INTO social_portfolio_invitations
            (owner_user_id, inviter_user_id, invited_email, invited_name, role, token)
            VALUES (:owner, :inviter, :email, :name, :role, :token)');
        $stmt->execute([
            'owner' => $ownerUserId,
    ...
    public static function hasValidInviteForEmail(int $ownerUserId, string $email): bool
    {
        $email = trim($email);
        if ($ownerUserId <= 0 || $email === '') {
            return false;
        }
        $pdo = Database::getConnection();
        $stmt = $pdo->prepare('SELECT 1 FROM social_portfolio_invitations
            WHERE owner_user_id = :owner AND invited_email = :email AND status = "pending"
            LIMIT 1');
        $stmt->execute([
            'owner' => $ownerUserId,
            'email' => $email,
    ...
    public static function allPendingForOwner(int $ownerUserId): array
    {
        ...
        $pdo = Database::getConnection();
        $stmt = $pdo->prepare('SELECT * FROM social_portfolio_invitations WHERE owner_user_id = :owner AND status = "pending" ORDER BY created_at DESC');
