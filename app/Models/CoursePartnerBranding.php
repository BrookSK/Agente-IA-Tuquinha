...
class CoursePartnerBranding
{
    ...
    public static function upsert(int $userId, array $data): void
    {
        ...
        $stmt = $pdo->prepare('INSERT INTO course_partner_branding (user_id, company_name, logo_url, primary_color, secondary_color)
            VALUES (:uid, :company_name, :logo_url, :primary_color, :secondary_color)');
        $stmt->execute([
            'uid' => $userId,
            'company_name' => ($data['company_name'] ?? '') !== '' ? (string)$data['company_name'] : null,
            'logo_url' => ($data['logo_url'] ?? '') !== '' ? (string)$data['logo_url'] : null,
