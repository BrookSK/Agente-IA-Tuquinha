...
class CoursePartnerBranding
{
    ...
    public static function upsert(int $userId, array $data): void
    {
        ...
        $stmt = $pdo->prepare('INSERT INTO course_partner_branding (user_id, company_name, logo_url, primary_color, secondary_color, text_color, button_text_color, header_image_url, footer_image_url, hero_image_url, background_image_url)
            VALUES (:uid, :company_name, :logo_url, :primary_color, :secondary_color, :text_color, :button_text_color, :header_image_url, :footer_image_url, :hero_image_url, :background_image_url)');
        $stmt->execute([
            'uid' => $userId,
            'company_name' => ($data['company_name'] ?? '') !== '' ? (string)$data['company_name'] : null,
            'logo_url' => ($data['logo_url'] ?? '') !== '' ? (string)$data['logo_url'] : null,
