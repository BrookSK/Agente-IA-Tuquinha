    ...
    <section style="background:#111118; border-radius:16px; border:1px solid #272727; padding:12px 14px;">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;">
            ...
            <div style="display:flex; align-items:center; gap:10px;">
                <a href="/painel-externo/amigos/adicionar" style="display:inline-block; font-size:12px; padding:6px 10px; border-radius:999px; background:linear-gradient(135deg,#e53935,#ff6f60); color:#050509; text-decoration:none; font-weight:700;">Adicionar amigo</a>
                <span style="font-size:12px; color:#b0b0b0;"><?= (int)$friendsCount ?> amigo(s)</span>
            </div>
        </div>

        <form action="/painel-externo/amigos" method="get" style="display:flex; gap:8px; align-items:center; flex-wrap:wrap; margin-top:8px;">
            ...
            <?php if ($q !== '' || $onlyFavorites): ?>
                <a href="/painel-externo/amigos" style="font-size:12px; color:#ff6f60; text-decoration:none;">Limpar</a>
            <?php endif; ?>
        </form>
            ...
            <div style="display:grid; grid-template-columns:repeat(auto-fill, minmax(160px, 1fr)); gap:10px; margin-top:8px;">
                <?php foreach ($friends as $f): ?>
                    ...
                    <div style="background:#050509; border-radius:12px; border:1px solid #272727; padding:8px 10px; display:flex; flex-direction:column; gap:6px;">
                        <a href="/painel-externo/perfil?user_id=<?= $friendId ?>" style="text-decoration:none; display:flex; align-items:center; gap:8px;">
                        ...
                        <div style="display:flex; gap:6px; flex-wrap:wrap;">
                            <a href="/painel-externo/chat?user_id=<?= $friendId ?>" style="display:inline-block; font-size:11px; padding:4px 8px; border-radius:999px; background:linear-gradient(135deg,#e53935,#ff6f60); color:#050509; text-decoration:none; font-weight:600;">
                                Conversar
                            </a>
                            <a href="/painel-externo/perfil?user_id=<?= $friendId ?>" style="display:inline-block; font-size:11px; padding:4px 8px; border-radius:999px; border:1px solid #272727; background:#111118; color:#f5f5f5; text-decoration:none; font-weight:600;">
                                Ver perfil
                            </a>
                            <?php if ($isFavorite): ?>
                            ...
                            <?php endif; ?>
                            <form action="/painel-externo/amigos/remover" method="post" onsubmit="return confirm('Tem certeza que deseja remover este amigo?');" style="display:inline; margin:0;">
                ...
                <?php foreach ($pending as $p): ?>
                    ...
                    <div style="background:#050509; border-radius:12px; border:1px solid #272727; padding:8px 10px; display:flex; align-items:center; justify-content:space-between; gap:10px;">
                        <div style="display:flex; align-items:center; gap:8px;">
                            ...
                            <div>
                                <a href="/painel-externo/perfil?user_id=<?= $otherId ?>" style="font-size:13px; font-weight:600; color:#f5f5f5; text-decoration:none;">
                                    <?= htmlspecialchars($otherName, ENT_QUOTES, 'UTF-8') ?>
                                </a>
                                <div style="font-size:11px; color:#b0b0b0;">Quer ser seu amigo</div>
                            </div>
                        </div>
                        <form action="/painel-externo/amigos/decidir" method="post" style="display:flex; gap:6px;">
