...
<div style="max-width: 900px; width: 100%; margin: 0 auto; padding: 0 8px; display: flex; flex-direction: column; min-height: calc(100vh - 56px - 80px); box-sizing: border-box;">
    ...
    <?php if (!empty($conversationId) && !empty($personaOptions) && is_array($personaOptions)): ?>
        <?php
            $currentPersonaId = isset($currentPersonaData['id']) ? (int)$currentPersonaData['id'] : 0;
            $canPickOnlyDefault = (!$planAllowsPersonalitiesFlag);
        ?>
        <div style="margin-top:10px; margin-bottom:6px; display:flex; justify-content:flex-start; gap:8px; align-items:center; flex-wrap:wrap;">
            <div style="font-size:12px; color:#b0b0b0; display:flex; gap:8px; align-items:center; flex-wrap:wrap;">
                ...
                <?php else: ?>
                    <span style="font-weight:500;">Personalidade:</span>
                    <span>Padrão do Tuquinha</span>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div style="margin-bottom:8px; display:flex; gap:8px; flex-wrap:wrap;">
            <?php foreach ($personaOptions as $po): ?>
                <?php
                    ...
                    if ($canPickOnlyDefault && $pid !== $defaultPersonaIdValue) {
                        $isAllowedToClick = false;
                ...
                <?php if ($planAllowsPersonalitiesFlag): ?>
                    <form action="/chat/persona" method="post" style="margin:0;">
                        <input type="hidden" name="conversation_id" value="<?= (int)$conversationId ?>">
                        <input type="hidden" name="persona_id" value="<?= $pid ?>">
                        <button type="submit" <?= !$isAllowedToClick ? 'disabled' : '' ?> title="<?= $disabledReason !== '' ? htmlspecialchars($disabledReason) : '' ?>" style="
                            border:1px solid var(--border-subtle);
                ...
                <?php else: ?>
                    <?php
                        $href = ($isAllowedToClick && $pid === $defaultPersonaIdValue)
                            ? '/chat?new=1'
                            : 'javascript:void(0)';
                    ?>
                    ...
                    ">
                        ...
                        <?= $isComingSoon ? ' • Em breve' : '' ?>
                        <?= $isDefault ? ' • Principal' : '' ?>
                        <?php if ($canPickOnlyDefault && $pid !== $defaultPersonaIdValue && !$isComingSoon): ?>
                            <span style="font-size:10px; color:#ffcc80;">(Plano pago)</span>
                        <?php endif; ?>
                    </a>
                <?php endif; ?>
            <?php endforeach; ?>
        ...
        <?php if ($canPickOnlyDefault): ?>
            <div style="margin:-2px 0 8px 0; font-size:11px; color:#8d8d8d;">
                Preview: no plano Free você vê as personalidades, mas só pode usar a personalidade padrão do Tuquinha.
            </div>
        <?php endif; ?>
    <?php endif; ?>
